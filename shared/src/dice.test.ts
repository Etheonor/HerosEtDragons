...
describe("parseDiceCommand", () => {
  it("parses /1d20", () => {
    expect(parseDiceCommand("/1d20")).toEqual({ n: 1, sides: 20, mod: 0, drop: 0 });
  });

  it("parses /2d6+3", () => {
    expect(parseDiceCommand("/2d6+3")).toEqual({ n: 2, sides: 6, mod: 3, drop: 0 });
  });

  it("parses /3d8-2", () => {
    expect(parseDiceCommand("/3d8-2")).toEqual({ n: 3, sides: 8, mod: -2, drop: 0 });
  });

  it("parses /d20 (implicit 1 die)", () => {
    expect(parseDiceCommand("/d20")).toEqual({ n: 1, sides: 20, mod: 0, drop: 0 });
  });

  it("parses /1d20+0", () => {
    expect(parseDiceCommand("/1d20+0")).toEqual({ n: 1, sides: 20, mod: 0, drop: 0 });
  });

  it("parses /20d100+5", () => {
    expect(parseDiceCommand("/20d100+5")).toEqual({ n: 20, sides: 100, mod: 5, drop: 0 });
  });

  it("parses /2d6 (no modifier)", () => {
    expect(parseDiceCommand("/2d6")).toEqual({ n: 2, sides: 6, mod: 0, drop: 0 });
  });
  ...
  it("handles negative modifier with large value", () => {
    expect(parseDiceCommand("/1d20-10")).toEqual({ n: 1, sides: 20, mod: -10, drop: 0 });
  });
...
describe("isCritical", () => {
  it("returns true for 1d20 natural 20", () => {
    const roll: DiceRoll = { n: 1, sides: 20, mod: 0, faces: [20], total: 20, dropped: [] };
    expect(isCritical(roll)).toBe(true);
  });

  it("returns false for 1d20 natural 19", () => {
    const roll: DiceRoll = { n: 1, sides: 20, mod: 0, faces: [19], total: 19, dropped: [] };
    expect(isCritical(roll)).toBe(false);
  });

  it("returns false for 2d20 even if one is 20", () => {
    const roll: DiceRoll = { n: 2, sides: 20, mod: 0, faces: [20, 5], total: 25, dropped: [] };
    expect(isCritical(roll)).toBe(false);
  });
});
...
describe("isFumble", () => {
  it("returns true for 1d20 natural 1", () => {
    const roll: DiceRoll = { n: 1, sides: 20, mod: 0, faces: [1], total: 1, dropped: [] };
    expect(isFumble(roll)).toBe(true);
  });

  it("returns false for 1d20 natural 2", () => {
    const roll: DiceRoll = { n: 1, sides: 20, mod: 0, faces: [2], total: 2, dropped: [] };
...
describe("formatRollDetail", () => {
  it("formats 1d20+7 = 14+7", () => {
    const roll: DiceRoll = { n: 1, sides: 20, mod: 7, faces: [14], total: 21, dropped: [] };
    expect(formatRollDetail(roll)).toBe("14 + 7");
  });

  it("formats 2d6+3 = 3+5+3", () => {
    const roll: DiceRoll = { n: 2, sides: 6, mod: 3, faces: [3, 5], total: 11, dropped: [] };
    expect(formatRollDetail(roll)).toBe("3 + 5 + 3");
  });

  it("formats 1d20 without modifier", () => {
    const roll: DiceRoll = { n: 1, sides: 20, mod: 0, faces: [14], total: 14, dropped: [] };
    expect(formatRollDetail(roll)).toBe("14");
  });

  it("formats 1d20-5 with negative modifier", () => {
    const roll: DiceRoll = { n: 1, sides: 20, mod: -5, faces: [14], total: 9, dropped: [] };
    expect(formatRollDetail(roll)).toBe("14 - 5");
  });

  it("formats multiple dice without modifier", () => {
    const roll: DiceRoll = { n: 3, sides: 6, mod: 0, faces: [2, 4, 1], total: 7, dropped: [] };
    expect(formatRollDetail(roll)).toBe("2 + 4 + 1");
  });
});

describe("biffer (drop lowest)", () => {
  it("parse 4d6b1", () => {
    expect(parseDiceCommand("/4d6b1")).toEqual({ n: 4, sides: 6, mod: 0, drop: 1 });
  });

  it("retire le dé le plus bas", () => {
    const roll = rollDice(4, 6, 0, fixedRng([4, 1, 5, 2]), 1);
    expect(roll.faces).toEqual([5, 6, 3]);
    expect(roll.dropped).toEqual([2]);
    expect(roll.total).toBe(14);
  });

  it("détail mentionne le dé biffé", () => {
    const roll = rollDice(4, 6, 0, fixedRng([4, 1, 5, 2]), 1);
    expect(formatRollDetail(roll)).toBe("5 + 6 + 3 (biffé 2)");
  });

  it("rejette un drop invalide (biffer tous les dés)", () => {
    expect(parseDiceCommand("/4d6b4")).toBeNull();
  });
});
